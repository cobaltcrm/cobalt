...
defined( '_JEXEC' ) or die( 'Restricted access' ); 
	...
	for ( $i=0; $i<$n; $i++ ){
		...
		if($event['type']=='task') {
			$due_date = $event['due_date']!= '0000-00-00 00:00:00' ? CobaltHelperDate::formatDate($event['due_date']) : CRMText::_('COBALT_NA');
		...
		echo '<tr id="list_row_'.$id.'" class="cobalt_row_'.$k.'">';
            ...
            echo '<td>';
	            if ( CobaltHelperUsers::getLoggedInUser()->id == $event['owner_id'] || CobaltHelperUsers::getLoggedInUser()->id == $event['assignee_id'] || CobaltHelperUsers::isAdmin() || CobaltHelperUsers::getRole() == "exec" ){
            ...
            echo $event['name'];
            echo '</a></td>';
			echo '<td class="due_date_column">'.$due_date.' '.CobaltHelperDate::formatTime($time,"(".CobaltHelperUsers::getTimeFormat().")").'</td>';
			echo '<td>';
			...
			echo '<td class="contacts" ><a href="javascript:void(0);" onclick="showEventContactsDialogModal('.$event['id'].');"><img src="'.JURI::base().'libraries/crm/media/images/card.png'.'"/></a></td>';
			echo '<td class="notes"><a href="javascript:void(0);" onclick="openNoteModal(\''.$event['id'].'\',\'event\');"><img src="'.JURI::base().'libraries/crm/media/images/notes.png'.'"/></a>';
