...
$stage_name = ( array_key_exists('stage_id',$deal) && $deal['stage_id'] != 0 ) ? $deal['stage_name'] : CRMText::_('COBALT_CLICK_TO_EDIT');
$source_name = ( array_key_exists('source_id',$deal) && $deal['source_id'] != 0 ) ? $deal['source_name'] : CRMText::_('COBALT_CLICK_TO_EDIT');
$expected_close = $deal['expected_close'] != "0000-00-00 00:00:00" ? CobaltHelperDate::formatDate($deal['expected_close']) : CRMText::_('COBALT_NOT_SET');
$actual_close = $deal['actual_close'] != "0000-00-00 00:00:00" && $deal['closed'] != 0 ? CobaltHelperDate::formatDate($deal['actual_close']) : $actual_close = CRMText::_('COBALT_ACTIVE_DEAL');
$extras = '<b>'.CRMText::_('COBALT_PRIMARY_CONTACT').'</b>
            <a href="'.JRoute::_('index.php?view=people&layout=person&id='.$deal['primary_contact_id']).'">'.$deal['primary_contact_first_name'].'</a><br>
            <b>'.CRMText::_('COBALT_NEXT_ACTION').'</b><br>';
?>
<tr id="list_row_<?php echo $deal['id']; ?>" >
    ...
    <td class="contacts_notes">
        <div class="btn-group">
            <a rel="tooltip" title="<?php echo CRMText::_('COBALT_VIEW_CONTACTS'); ?>" data-placement="bottom" class="btn" href="javascript:void(0);" onclick="showDealContactsDialogModal(<?php echo $deal['id']; ?>);"><i class="icon-user"></i></a>
            <a rel="tooltip" title="<?php echo CRMText::_('COBALT_VIEW_NOTES'); ?>" data-placement="bottom" class="btn" href="javascript:void(0);" onclick="openNoteModal(<?php echo $deal['id']; ?>,'deal');"><i class="icon-file"></i></a>
