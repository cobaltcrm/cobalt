...
<div class="media" id="note_entry_<?php echo $note['id']; ?>">
    <?php if ( CobaltHelperUsers::getRole() == "exec" || $note['author'] == CobaltHelperUsers::getUserId() ) { ?>
    ...
    <div class="media-body">
        ...
        <div>
            <small>
            <?php echo CRMText::_('COBALT_WROTE_NOTE');
            if ( array_key_exists('category_name',$note) && $note['category_name'] !=  "" ) {
                echo CRMText::_('COBALT_IN').'<b> '.$note['category_name'].'</b> ';
            }
            if ( array_key_exists('person_first_name',$note) && $note['person_first_name'] !=  "" ) {
                echo CRMText::_('COBALT_FOR').' <b>'.$note['person_first_name'].' '.$note['person_last_name'].'</b> ';
            }
            if ( array_key_exists('deal_name',$note) && $note['deal_name'] !=  ""  ) {
                echo CRMText::_('COBALT_ON_THE_DEAL').' <b>'.$note['deal_name'].'</b>';
            }
            if ( array_key_exists('event_name',$note) && $note['event_name'] !=  "" ) {
                echo CRMText::_('COBALT_ON_THE_EVENT').' <b>'.$note['event_name'].'</b>';
            }
