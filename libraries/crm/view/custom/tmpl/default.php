...
defined( '_CEXEC' ) or die( 'Restricted access' );
    ...
    foreach ($custom as $field => $value) {
        ...
        switch ($value['type']) {
            ...
            break;
            case "date":
                $custom_field_filter = array_key_exists('selected',$value) && $value['selected'] != 0 && $value['selected'] != "0000-00-00 00:00:00" ? CobaltHelperDate::formatDate($value['selected']) : CRMText::_('COBALT_CLICK_TO_EDIT');
            break;
            case "picklist":
                $custom_field_filter = ( is_array($value) && array_key_exists('values',$value) && is_array($value['values']) && array_key_exists('selected',$value) && array_key_exists($value['selected'],$value['values']) ) ? $value['values'][$value['selected']] : CRMText::_('COBALT_CLICK_TO_EDIT');
            break;
        }
            echo '<tr>';
                ...
                switch ($value['type']) {
                    ...
                    <span class="editable parent" id="editable_custom_<?php echo $value['id']; ?>_container">
                        <div class="inline" id="editable_custom_<?php echo $value['id']; ?>">
                            <a href="javascript:void(0);" rel="popover" data-title="<?php echo CRMText::_('COBALT_UPDATE_FIELD').' '.$value['name']; ?>" data-html='true' data-content='<div class="clearfix input-append"><form id="<?php echo $value['id']; ?>_form">
                            <input placeholder="<?php echo CRMText::_('COBALT_CLICK_TO_EDIT'); ?>" type="text" class="inputbox input-small" name="custom_<?php echo $value['id']; ?>" value="<?php echo $value['selected']; ?>" />
                    ...
                    <!-- make this a custom date picker -->
                        <form name="custom_<?php echo $value['id'];?>_form" id="custom_<?php echo $value['id']; ?>_form">
                            <div class="input-append">
                                <input class="input-small inputbox-hidden date_input editable-modal-datepicker" id="custom_<?php echo $value['id']; ?>" name="custom_<?php echo $value['id']; ?>_hidden" type="text" placeholder="<?php echo CRMText::_('COBALT_CLICK_TO_EDIT'); ?>"  value="<?php echo $custom_field_filter; ?>"  />
