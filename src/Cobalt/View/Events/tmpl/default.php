...
<div>
    <ul class="filter_lists">
        <li class="filter_sentence">
            <?php echo TextHelper::_('COBALT_SHOW'); ?>
            ...
            <?php
                $arr = array('any'=>TextHelper::_('COBALT_ANY_TYPE'));
            ...
            <?php echo TextHelper::_('COBALT_THAT_ARE'); ?>
            ...
            <?php echo TextHelper::_('COBALT_FOR'); ?>
            ...
            <?php echo TextHelper::_('COBALT_ASSIGNED_TO'); ?>
            <?php
                $arr = array(array('value'=>UsersHelper::getUserId(),'label'=>TextHelper::_('COBALT_ME')));
                $users = UsersHelper::getUsers();
                $teams = UsersHelper::getTeams();
                if ( UsersHelper::getRole() != 'basic' ) {
                    $arr[]=array('value'=>'all','label'=>TextHelper::_('COBALT_ALL_USERS'));
                }
                ...
                <ul>
                    ...
                    <?php if ($user_role != 'basic') { ?>
                        <?php if ($user_role == "exec") { ?>
                            <li><a class="dropdown_item filter_user_all" onclick="eventUserFilter('individual','all')" ><?php echo ucwords(TextHelper::_('COBALT_ALL_USERS')); ?></a></li>
                        <?php } ?>
                        <?php if ($user_role == "manager") { ?>
                            <li><a class="dropdown_item filter_user_all" onclick="eventUserFilter('individual','all')" ><?php echo ucwords(TextHelper::_('COBALT_ALL_USERS_ON_MY_TEAM')); ?></a></li>
                        <?php } ?>
                    <?php } ?>
                    <?php
                        if ($user_role == 'exec') {
                            if ( count($teams) > 0 ) {
                                foreach ($teams as $team) {
                                     echo "<li><a class='dropdown_item filter_team_".$team['team_id']."' onclick='eventUserFilter(\"team\",".$team['team_id'].")'>".$team['team_name'].TextHelper::_('COBALT_TEAM_APPEND')."</a></li>";
                                 }
                            }
                        }
                        if ( count($users) > 0 ) {
    ...
    <div class="actions_container">
        <span class="actions">
            <a onclick="addTaskEvent('task')"><?php echo ucwords(TextHelper::_('COBALT_ADD_TASK')); ?></a> -
            <a onclick="addTaskEvent('event')"><?php echo ucwords(TextHelper::_('COBALT_ADD_EVENT')); ?></a> -
            <a href="<?php echo JRoute::_('index.php?view=calendar'); ?>" ><?php echo TextHelper::_('COBALT_SHOW_CALENDAR'); ?></a> -
            <a href="javascript:void(0)" onclick="printItems('event_form');"><?php echo TextHelper::_('COBALT_PRINT'); ?></a>
        </span>
    </div>
...
<table id='events_list' class="com_cobalt_table">
       <thead>
               <tr>
                ...
                <th><?php echo TextHelper::_('COBALT_EVENTS_FOR'); ?></th>
                <th><?php echo TextHelper::_('COBALT_CREATED_BY'); ?></th>
                <th><?php echo TextHelper::_('COBALT_ASSIGNED_TO'); ?></th>
                ...
                <th><?php echo TextHelper::_('COBALT_EVENTS_CONTACTS'); ?></th>
                <th><?php echo TextHelper::_('COBALT_EVENTS_NOTES'); ?></th>
            </tr>
        </thead>
        <tbody id="events">
            <?php
