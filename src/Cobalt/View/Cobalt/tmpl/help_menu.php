...
<?php if ($this->help_type != "launch_default") { if (!$this->launch_default) { ?>
    <div class="panel panel-warning">
        <div class="panel-heading">
            <h4 class="panel-title"><?php echo TextHelper::_('COBALT_QUICK_START'); ?></h4>
        </div>
...
<?php if ($this->show_update_buttons || $this->show_help) {  ?>
            ...
            <div class="alert alert-info">
                <?php if (!$this->show_update_buttons) { ?>
                    <div class="pull-right">
                        <a class="btn-mini btn btn-danger" data-toggle="modal" href="#disable_help_hidden" onclick="disableHelp();" id="disable_help_button"><?php echo TextHelper::_('COBALT_DISABLE_HELP'); ?></a>
                    </div>
                <?php } ?>
                <h4><?php echo TextHelper::_('COBALT_HELP_'.strtoupper($this->help_type).'_TITLE'); ?></h4>
                <?php echo TextHelper::_('COBALT_HELP_'.strtoupper($this->help_type).'_DESC'); ?>
            </div>
            <?php if (!$this->step_completed && $this->show_update_buttons) { ?>
                <div id="help_actions" class="pull-right btn-group">
                    <a href="javascript:void(0);" onclick="updateConfig('<?php echo $this->help_type; ?>',1);" class="btn-primary btn-large" ><i class="glyphicon glyphicon-check icon-white"></i><?php echo TextHelper::_('COBALT_COMPLETE'); ?></a>
                    <a href="javascript:void(0);" onclick="updateConfig('<?php echo $this->help_type; ?>',0);" class="btn laterButton" ><i class="glyphicon glyphicon-repeat"></i><?php echo TextHelper::_('COBALT_COMPLETE_LATER'); ?></a>
                </div>
            <?php } ?>
        </form>
    </div>
    <div class="modal hide fade in" id="disable_help_hidden">
        <div class="modal-header">
            ...
            <h3><?php echo TextHelper::_('COBALT_DISABLE_HELP_TITLE'); ?></h3>
        </div>
        <div class="modal-body">
            <p><?php echo TextHelper::_('COBALT_DISABLE_HELP_DESC'); ?></p>
        </div>
