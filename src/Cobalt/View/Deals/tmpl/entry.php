...
$expected_close = $deal->expected_close != Factory::getDb()->getNullDate() ? DateHelper::formatDate($deal->expected_close) : TextHelper::_('COBALT_NOT_SET');
$actual_close   = $deal->actual_close != Factory::getDb()->getNullDate() && $deal->closed != 0 ? DateHelper::formatDate($deal->actual_close) : $actual_close = TextHelper::_('COBALT_ACTIVE_DEAL');
$extras         = '<b>' . TextHelper::_('COBALT_PRIMARY_CONTACT') . '</b><a href="' . RouteHelper::_('index.php?view=people&layout=person&id=' . $deal->primary_contact_id) . '">' . $deal->primary_contact_first_name . '</a><br /><b>' . TextHelper::_('COBALT_NEXT_ACTION') . '</b><br />';
?>
<tr id="list_row_<?php echo $deal->id; ?>">
	...
	<td class='status'>
		<div class='dropdown'>
			...
			<ul class="dropdown-menu" aria-labelledby="deal_stage_<?php echo $deal->id; ?>" role="menu">
				<?php if (isset($statuses) && count($statuses)) :
					foreach ($statuses as $id => $class) : ?>
						<li>
	...
	<td class="stage">
		<div class='dropdown'>
			...
			<ul class="dropdown-menu" aria-labelledby="deal_stage_<?php echo $deal->id; ?>" role="menu">
				<?php if (isset($stages) && count($stages)) :
					foreach ($stages as $id => $stage) : ?>
						<li>
							<a href="javascript:void(0)" class="stage_select dropdown_item" data-field="stage_id" data-item="deal" data-item-id="<?php echo isset($deal->id) ? $deal->id : ''; ?>" data-value="<?php echo isset($stage['id']) ? $stage['id'] : ''; ?>"><?php echo $stage['name']; ?>
								<div class="status-dot" style="background-color: #<?php echo $stage['color']; ?>"></div>
							</a>
						</li>
					<?php endforeach;
				endif; ?>
	...
	<td class="source">
		<div class='dropdown'>
			<a href='javascript:void(0);' class='dropdown-toggle update-toggle-html' role='button' data-toggle='dropdown' id='deal_source_<?php echo $deal->id; ?>_link'><?php echo $source_name; ?></a>
			<ul class="dropdown-menu" aria-labelledby="deal_source_<?php echo $deal->id; ?>" role="menu">
				<?php if (count($sources)) :
					foreach ($sources as $id => $source) : ?>
						<li>
							<a href="javascript:void(0)" class="source_select dropdown_item" data-field="source_id" data-item="deal" data-item-id="<?php echo $deal->id; ?>" data-value="<?php echo $id; ?>"><?php echo $source; ?></a>
						</li>
					<?php endforeach;
				endif; ?>
			</ul>
