 ...
 * @since       11.1
 */
class JTableUser extends Cobalt\Table\AbstractTable
{
    public $data;
    ...
    public function load($userId = null, $reset = true)
    {
        ...
        $query->where($this->db->quoteName('id') . ' = ' . (int) $userId);
        $this->db->setQuery($query);
        $data = (array) $this->db->loadAssoc();
        ...
        if ($return !== false) {
            ...
            $query->clear();
            $query->select($this->db->quoteName('g.id'));
            $query->select($this->db->quoteName('g.title'));
            $query->from($this->db->quoteName('#__usergroups') . ' AS g');
            $query->join('INNER', $this->db->quoteName('#__user_usergroup_map') . ' AS m ON m.group_id = g.id');
            $query->where($this->db->quoteName('m.user_id') . ' = ' . (int) $userId);
            $this->db->setQuery($query);

            // Add the groups to the user data.
            $this->groups = $this->db->loadAssocList('id', 'id');
        }
    ...
    public function bind($array, $ignore = '')
    {
        ...
        if ($return && !empty($this->groups)) {
            ...
            $query->where($this->db->quoteName('id') . ' = ' . implode(' OR ' . $this->db->quoteName('id') . ' = ', $this->groups));
            $this->db->setQuery($query);
    ...
    public function check()
    {
        ...
        if (empty($this->registerDate) || $this->registerDate == $this->db->getNullDate()) {
            $this->registerDate = JFactory::getDate()->toSql();
        }

        // Set the lastvisitDate timestamp
        if (empty($this->lastvisitDate)) {
            $this->lastvisitDate = $this->db->getNullDate();
        }
        ...
        $query->select($this->db->quoteName('id'));
        $query->from($this->db->quoteName('#__users'));
        $query->where($this->db->quoteName('username') . ' = ' . $this->db->quote($this->username));
        $query->where($this->db->quoteName('id') . ' != ' . (int) $this->id);
        $this->db->setQuery($query);

        $xid = (int) $this->db->loadResult();
        ...
        $query->select($this->db->quoteName('id'));
        $query->from($this->db->quoteName('#__users'));
        $query->where($this->db->quoteName('email') . ' = ' . $this->db->quote($this->email));
        $query->where($this->db->quoteName('id') . ' != ' . (int) $this->id);
        $this->db->setQuery($query);
        $xid = (int) $this->db->loadResult();
        if ($xid && $xid != (int) $this->id) {
        ...
        if (!is_numeric($rootUser)) {
            ...
            $query->select($this->db->quoteName('id'));
            $query->from($this->db->quoteName('#__users'));
            $query->where($this->db->quoteName('username') . ' = ' . $this->db->quote($rootUser));
            $this->db->setQuery($query);
            $xid = (int) $this->db->loadResult();
    ...
    public function store($updateNulls = false)
    {
        ...
        if ($key) {
            ...
            $this->db->updateObject($this->_tbl, $this, $this->_tbl_key, $updateNulls);
        } else {
            // Don't have a table key, insert the row.
            $this->db->insertObject($this->_tbl, $this, $this->_tbl_key);
        }
        ...
        if (is_array($this->groups) && count($this->groups)) {
            ...
            $query->where($this->db->quoteName('user_id') . ' = ' . (int) $this->id);
            $this->db->setQuery($query);
            $this->db->execute();
            ...
            $query->columns(array($this->db->quoteName('user_id'), $this->db->quoteName('group_id')));
    ...
    public function delete($userId = null)
    {
        ...
        $query = $this->db->getQuery(true);
        $query->delete();
        $query->from($this->db->quoteName($this->_tbl));
        $query->where($this->db->quoteName($this->_tbl_key) . ' = ' . (int) $this->$k);
        $this->db->setQuery($query);
        $this->db->execute();
        ...
        $query->delete();
        $query->from($this->db->quoteName('#__user_usergroup_map'));
        $query->where($this->db->quoteName('user_id') . ' = ' . (int) $this->$k);
        $this->db->setQuery($query);
        $this->db->execute();
        ...
        $query->clear();
        $query->delete();
        $query->from($this->db->quoteName('#__messages_cfg'));
        $query->where($this->db->quoteName('user_id') . ' = ' . (int) $this->$k);
        $this->db->setQuery($query);
        $this->db->execute();
        ...
        $query->clear();
        $query->delete();
        $query->from($this->db->quoteName('#__messages'));
        $query->where($this->db->quoteName('user_id_to') . ' = ' . (int) $this->$k);
        $this->db->setQuery($query);
        $this->db->execute();
